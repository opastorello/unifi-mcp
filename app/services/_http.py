...
# Teto de espera ao honrar o header Retry-After de um 429, para não pendurar a
# tool indefinidamente caso o upstream peça um intervalo muito longo.
_RETRY_AFTER_MAX = 30.0


def _parse_retry_after(value: str | None) -> float | None:
    """Converte o header `Retry-After` em segundos (>=0). Suporta os dois formatos
    da RFC 7231: delta-seconds (ex.: "5") e HTTP-date. Retorna None se ausente/inválido.

    A doc da UniFi (Site Manager → Rate Limiting) define 429 + Retry-After "indicating
    the number of seconds to wait before making additional requests".
    """
    if not value:
        return None
    value = value.strip()
    try:  # delta-seconds (a UniFi pode mandar com fração)
    ...
    async with _get_sem(device_id):
        ...
        try:
            for attempt in range(_cfg.MAX_RETRIES + 1):
                ...
                wait: float | None = None  # espera explícita (429 Retry-After)
                try:
                    ...
                    if resp.status_code == 429:
                        # Rate limit: honra o Retry-After (capado) antes de tentar de novo.
                        _m.upstream_errors_total.labels(
                            type="rate_limit", device=device_id
                        ).inc()
                        last = UpstreamError("upstream HTTP 429 (rate limit)")
                        ra = _parse_retry_after(resp.headers.get("Retry-After"))
                        if ra is not None:
                            wait = min(ra, _RETRY_AFTER_MAX)
                ...
                if attempt < _cfg.MAX_RETRIES:
                    await asyncio.sleep(wait if wait is not None else 0.5 * (attempt + 1))
        finally:
